 ...
 * one %-tuning slider (locator coverage), and two sub-toggles (edge locators,
 * central locators). Mutually exclusive with flat/lightweight bases and
 * magnet/screw holes — see useSparseSection.
 */
...
export function SparseSection() {
  ...
  return (
    ...
    >
      ...
      />
      <SliderInput
        label={t('binDesigner.sparseBaseLocatorCoverage')}
        value={sparseBase.locatorCoverage}
        onChange={handlers.setLocatorCoverage}
        min={DESIGNER_CONSTRAINTS.MIN_SPARSE_LOCATOR_COVERAGE}
        max={DESIGNER_CONSTRAINTS.MAX_SPARSE_LOCATOR_COVERAGE}
        step={DESIGNER_CONSTRAINTS.SPARSE_LOCATOR_COVERAGE_STEP}
        unit="%"
      />
